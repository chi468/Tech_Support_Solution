...
<html>
  ...
  <body>
    ...
    <main role="main">
	  ...
	  <c:if test="${sessionScope.member.typeNo >= 2}">
        <div style="text-align:right">    
	  ...
	  <c:if test="${sessionScope.member.typeNo > 2}">
        <div style="text-align:right">    
	  			<button id="managingForce" data-toggle="modal"  data-target="#memberListModal" class="btn btn-primary my-2" style="background-color:DodgerBlue; color:white; border:0px; border-radius:10px;">담당자 강제지정</button>
	    </div>
	  </c:if>
  ...
  <div class="modal fade" id="stateModal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <!-- Modal body -->
    	<form action="/changeState.do" method="post">
        <div class="modal-body" class="col-sm-2 mb-3">
        
          		<select id="stateNo" name="stateNo" class="form-control">
          			<option value="2">본사직원 할당완료</option>
          			<option value="3">분석 중</option>
        	  		<option value="4">추가 정보요청</option>
    	      		<option value="5">피드백필요</option>    	      		
	          		<option value="6">해결완료</option>	          		
          		</select>
          		<input type="hidden" name="boardNo" value="${requestScope.board.boardNo}"/>
   ...
   <!-- The Modal -->
  <div class="modal fade" id="memberListModal">
    <div class="modal-dialog modal-lg">
     <form action="/manageForce.do" method="post"> 
      <div class="modal-content">
        ...
        <div class="modal-body" class="col-sm-2 mb-3">       
           <div class="col-lg-12">
              <div class="card">
                ...
                <div class="card-body">
                  <div class="table-responsive">
                    <table class="table table-striped table-sm">
                      ...
                      <tbody>
                      	<c:forEach var="member" items="${requestScope.manuEngineerList}" varStatus="status">
                      		<tr id="manuEngineerList${status.count}">
                    	  		...
                    	  		<td>
                    	  			<c:if test="${requestScope.manuEngineerCountList[status.index] eq null}">
                    	  				0
                    	  			</c:if>
                    	  		${requestScope.manuEngineerCountList[status.index]}
                    	  		</td>                   	  		
                      		</tr>
                      		
                      		<script>
                      					$("#manuEngineerList${status.count}").click(function(){											
											$(this).css("border","outset");	
											$(this).siblings("tr").css("border","none");
											$("#manageMemberNo").val("${member.memberNo}");
										});                     		
							</script>                      	
                      	</c:forEach>    
                      </tbody>
       		...
       		<input type="hidden" name="memberNo" id="manageMemberNo"/>
        	<input type="hidden" name="boardNo" value="${requestScope.board.boardNo}"/>
        </div>
        ...
        <!-- Modal footer -->
        <div class="modal-footer">
        	<button type="submit" onclick="return memberNoCheck();" class="btn btn-secondary">완료</button>
          	<button type="reset" class="btn btn-secondary" data-dismiss="modal">취소</button>
        </div>
      </div>
     </form>
    ...
    <script>
		...
		if("${requestScope.board.stateNo}">1){
			...
			$("#managingForce").css("display","none");
		}
		...
		function manage(){
	 	...
	 	$("#stateNo").val("${requestScope.board.stateNo}");
		
		function memberNoCheck(){
			if($("#manageMemberNo").val()==""){
				alert("담당자 선택 후 완료 버튼을 눌러주세요");
				return false;
			}
			else
				return true;
